...
2. **En el lugar de consumo** (quién recibe el status report).
    - Pros: el parsing vive junto al comando que lo produce.
    - Cons: requiere acceso al estado previo desde fuera de `GrblStatus`.
...
- **[-]** La superficie de la API pública aumenta (2 nuevas clases: `GrblCommunicator` y `GrblInitializer`); la curva de aprendizaje para nuevos colaboradores es algo mayor.
...
2. Crear `GrblInitializer` en `src/core/core/utilities/grbl/grblInitializer.py`.
3. Actualizar `GrblStatus.update_status()` con carry-forward de WCO/overrides.
4. Simplificar `GrblController` para delegar en las nuevas clases.
5. Migrar y ampliar los tests correspondientes.
