...
class TestTaskRepository:
    ...
    def test_update_task(self, mocked_session):
        ...
        updated_task = task_repository.update_task(
            1, user_id, file_id, tool_id, material_id, name, note, priority
        )
    ...
    def test_update_task_status(
        self, mocked_session, status, new_status, expected_admin_id, expected_cancellation_reason
    ):
        task_repository = TaskRepository(mocked_session)
        admin_id = 1
        ...
        updated_task = task_repository.update_task_status(
            task.id, new_status, admin_id, cancellation_reason
        )
    ...
    @pytest.mark.parametrize(
        "status", ["pending_approval", "on_hold", "in_progress", "finished", "failed", "cancelled"]
    )
    ...
    def test_error_update_non_valid_transition(self, mocked_session, status, new_status):
        task_repository = TaskRepository(mocked_session)
