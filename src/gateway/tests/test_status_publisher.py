...
def make_publisher(
    *,
    ...
    StatusPublisher, FakeController, FakeFileExecutor, FakeSessionManager, MagicMock, MagicMock
]:
    ...
    return publisher, controller, file_executor, session_manager, store_mock, pubsub_mock
...
class TestGatewayStateProp:
    ...
    def test_setter_persists_to_store(self):
        publisher, *_, store_mock, _ = make_publisher()
        publisher.gateway_state = GW_STATE_STREAMING
        store_mock.set.assert_called_with(GATEWAY_STATE_KEY, GW_STATE_STREAMING)
...
class TestPublishIfDue:
    def test_not_publishes_before_interval(self):
        publisher, *_, _, pubsub_mock = make_publisher()
    ...
    def test_publishes_when_interval_elapsed(self):
        ...
        pubsub_mock.publish.assert_called()
...
class TestPublishNow:
    ...
    def test_persists_snapshot(self):
        publisher, *_, store_mock, _ = make_publisher()
        publisher.publish_now()
        # store.set is called for LAST_STATUS_KEY
        set_keys = [call.args[0] for call in store_mock.set.call_args_list]
...
class TestCleanup:
    def test_deletes_keys(self):
        ...
        publisher, *_, store_mock, _ = make_publisher()
        publisher.cleanup()
        deleted_keys = [call.args[0] for call in store_mock.delete.call_args_list]
...
class TestPublishStatusPayload:
    def _get_published_payload(self, pubsub_mock: MagicMock) -> dict:
        raw = pubsub_mock.publish.call_args.args[1]
        return json.loads(raw)

    def test_payload_has_required_keys(self):
        publisher, *_, _, pubsub_mock = make_publisher()
        publisher.publish_now()
        payload = self._get_published_payload(pubsub_mock)
    ...
    def test_session_comes_from_session_manager(self):
        ...
        publisher, *_, _, pubsub_mock = make_publisher(active_session=session)
        publisher.publish_now()
        payload = self._get_published_payload(pubsub_mock)
        assert payload["session"] == session

    def test_session_is_none_without_active_session(self):
        publisher, *_, _, pubsub_mock = make_publisher(active_session=None)
        publisher.publish_now()
        payload = self._get_published_payload(pubsub_mock)
