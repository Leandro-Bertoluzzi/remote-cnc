...
class TestTasksView:
    ...
    def setup_method(self, qtbot: QtBot, mocker: MockerFixture, mock_window: MainWindow):
        ...
        self.mock_get_all_tasks = mocker.patch.object(
            TaskRepository, "get_all_tasks_from_user", return_value=self.tasks_list
        )
    ...
    def test_tasks_view_init_with_no_tasks(self, mocker: MockerFixture, helpers):
        mock_get_all_tasks = mocker.patch.object(
            TaskRepository, "get_all_tasks_from_user", return_value=[]
        )
    ...
    def test_tasks_view_init_db_error(
        self, mocker, helpers, files_error, materials_error, tools_error, tasks_error
    ):
        mock_get_all_files = mocker.patch.object(
            FileRepository, "get_all_files_from_user", return_value=[]
        )
        if files_error:
            mock_get_all_files = mocker.patch.object(
                FileRepository, "get_all_files_from_user", side_effect=Exception("mocked-error")
            )
        ...
        if tasks_error:
            mock_get_all_tasks = mocker.patch.object(
                TaskRepository, "get_all_tasks_from_user", side_effect=Exception("mocked-error")
            )
    ...
    def test_tasks_view_create_task(self, mocker: MockerFixture, helpers):
        ...
        mock_create_task = mocker.patch.object(
            TaskRepository, "create_task", side_effect=side_effect_create_task
        )
