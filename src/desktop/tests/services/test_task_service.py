...
class TestTaskService:
    @staticmethod
    def _make_service_and_repo(mocker: MockerFixture, worker=None):
        session = MagicMock()
        ...
        service = TaskService(worker=worker_mock, session_factory=session_factory)
        return service, session, repository, worker_mock

    def test_get_all_tasks(self, mocker: MockerFixture):
        service, session, repository, _ = self._make_service_and_repo(mocker)
    ...
    def test_send_task_to_worker(self, mocker: MockerFixture):
        ...
        service, _, _, _ = self._make_service_and_repo(mocker, worker=worker)
    ...
    def test_create_and_execute_task(self, mocker: MockerFixture):
        ...
        service, session, repository, _ = self._make_service_and_repo(mocker, worker=worker)
