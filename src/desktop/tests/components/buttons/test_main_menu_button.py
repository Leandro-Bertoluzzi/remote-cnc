import pytest
from desktop.components.buttons.MainMenuButton import MainMenuButton, QPainter, QSvgRenderer
...
class TestMainMenuButton:
    @pytest.mark.parametrize("image_name", ["image.png", "image.svg"])
    def test_main_menu_button_init(self, qtbot: QtBot, mocker: MockerFixture, image_name):
        # Mock SVG creation
    ...
    @pytest.mark.parametrize(
        "image_name,is_hover",
        [("image.png", False), ("image.svg", False), ("image.png", True), ("image.svg", True)],
    )
    def test_main_menu_button_paint_event(
        self, qtbot: QtBot, mocker: MockerFixture, image_name, is_hover
    ):
        # Mock SVG creation
        mocker.patch.object(QSvgRenderer, "__init__", return_value=None)
    ...
    def test_main_menu_button_go_to_view(self, qtbot: QtBot, mocker: MockerFixture):
        ...
        # Instantiate button
        button = MainMenuButton("Test Button", "image.png", goToView="Test view", parent=parent)  # type: ignore[arg-type]
        qtbot.addWidget(button)
